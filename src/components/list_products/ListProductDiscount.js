...
function ListProductsDiscount() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let apiUrl = 'http://localhost:8080/AppElCerro-master/resources/detallexpromo/detallesxpromos';
        if (tipo) {
          apiUrl = `http://localhost:8080/AppElCerro-master/resources/detallexpromo/detallesxpromos/${tipo}`;
        }
